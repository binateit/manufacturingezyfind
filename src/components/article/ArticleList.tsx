...
export default function ArticleList({
    ...
    const [fetchArticleList, { data, loading }] = useLazyQuery(GET_POST_LIST, {
        fetchPolicy: "network-only",
    });

    useEffect(() => {
        if (data?.getPostList) {
            setArticles(data.getPostList.result ?? []);
            setPagination({
    ...
    const handleSearch = (value: string) => {
        setFilters((prev) => {
            ...
            fetchArticleList({ variables: buildVariables(next) });
            return next;
        });
    };
    ...
    return (
        <div className="flex flex-wrap lg:gap-6 lg:flex-nowrap my-10">
            ...
            <div className="basis-12/12 lg:basis-8/12 xl:basis-9/12 mt-5 lg:mt-0">
                ...
                {/* Article Cards */}
                {articles.length === 0 && (<NoRecordsCard />)}
                            ...
                            <div className="basis-12/12 sm:basis-8/12">
                                ...
                                {article.descriptionSEO ? (
                                    <div
                                        ...
                                        dangerouslySetInnerHTML={{ __html: article.descriptionSEO }}
                                    />
                                ) : (
