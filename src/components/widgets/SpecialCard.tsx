...
export default function SpecialCard({ companyId, companyName }: SpecialCardProps) {
    ...
    const specialProducts = data?.getPrdProductList?.result ?? [];
    const count = specialProducts.length ?? 0;
        ...
        <div>
            <div className='sm:flex items-center justify-between mt-10 sm:mt-6 '>
                <p className='text-primary text-center sm:text-left uppercase text-xl sm:text-2xl font-semibold mb-2 sm:mb-5 sm:w-[80%]'>
                    Special By {companyName}
                </p>
            </div>
            <Slider {...sliderSetting}>
                {specialProducts.map((product: ProductItem, index: number) => {
